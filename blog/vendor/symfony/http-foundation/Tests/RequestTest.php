...
class RequestTest extends TestCase
{
    ...
    public function testGetUri()
    {
        ...
        $server['REQUEST_URI'] = '/server.php/path/info?query=string';
        $server['SCRIPT_NAME'] = '/server.php';
        $server['PATH_INFO'] = '/path/info';
        $server['PATH_TRANSLATED'] = 'redirect:/server.php/path/info';
        $server['PHP_SELF'] = '/index_dev.php/path/info';
        $server['SCRIPT_FILENAME'] = '/some/where/server.php';

        $request = new Request();

        $request->initialize([], [], [], [], [], $server);

        $this->assertEquals('http://host:8080/server.php/path/info?query=string', $request->getUri(), '->getUri() with non default port');
        ...
        $this->assertEquals('http://host/server.php/path/info?query=string', $request->getUri(), '->getUri() with default port');
        ...
        $this->assertEquals('http://servername/server.php/path/info?query=string', $request->getUri(), '->getUri() with default port without HOST_HEADER');

        // Request with URL REWRITING (hide server.php)
        ...
        $server = [
            ...
            'PATH_TRANSLATED' => 'redirect:/server.php/foo bar/in+fo',
    ...
    public function testGetUriForPath()
    {
        ...
        $this->assertEquals('http://host:8080/server.php/some/path', $request->getUriForPath('/some/path'), '->getUriForPath() with non default port');
        ...
        $this->assertEquals('http://host/server.php/some/path', $request->getUriForPath('/some/path'), '->getUriForPath() with default port');
        ...
        $this->assertEquals('http://servername/server.php/some/path', $request->getUriForPath('/some/path'), '->getUriForPath() with default port without HOST_HEADER');
    ...
    public function testGetScriptName()
    {
        ...
        $this->assertEquals('/server.php', $request->getScriptName());
    ...
    public function testGetBasePath()
    {
        ...
        $server = [];
        $server['SCRIPT_FILENAME'] = '/some/where/server.php';
        $server['ORIG_SCRIPT_NAME'] = '/server.php';
        $request->initialize([], [], [], [], [], $server);
    ...
    public function getBaseUrlData()
    {
        return [
            [
                ...
                [
                    'SCRIPT_FILENAME' => 'E:/Sites/cc-new/public_html/fruit/server.php',
                    'SCRIPT_NAME' => '/fruit/server.php',
                    'PHP_SELF' => '/fruit/server.php',
                ],
                '/fruit',
            ...
            [
                '/fruit/strawberry/1234index.php/blah',
                [
                    'SCRIPT_FILENAME' => 'E:/Sites/cc-new/public_html/server.php',
